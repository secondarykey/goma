    ...
    private void collectWordInfo(String path, Searcher wid, String prefix,
        ...
        try {
            for (String s = rl.read(); s != null; s = rl.read()) {
                ...
                if (p2 == -1)
                    throw parseException("Word left context id must be terminated with '" + delim + "'.", path, rl);
                if (p3 == -1)
                    throw parseException("Word right context id must be terminated with '" + delim + "'.", path, rl);
                ...
                if (id < 0)
                    throw parseException("Word '" + s.substring(0, p1) + "' is unregistered in goma.dictionary.trie", path, rl);
