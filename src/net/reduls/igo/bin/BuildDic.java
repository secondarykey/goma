...
public final class BuildDic {
    public static void main(String[] args) {
        if (args.length != 3 && args.length != 4) {
            System.err.println("Usage: java net.reduls.goma.bin.BuildDic <output directory> <input directory> <encoding> [delimiter]");
            System.exit(1);
        }
        ...
        try {
            System.err.println("### Build word goma.dictionary.trie");
            wd.buildWordIdMap();

            System.err.println("### Build word goma.dictionary");
            wd.buildWordInfo();

            System.err.println("### Build matrix");
            Matrix.build(inputDir, outputDir);

            System.err.println("### Build char-category goma.dictionary");
            cc.build();
